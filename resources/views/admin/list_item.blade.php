...
@section('subjudul','Data Log Item')
@section('content')
<div class="float-end">
...
<div class="modal" id="modal-detail" tabindex="-1" aria-hidden="true">
        ...
        <div class="row g-9 mb-8">
                ...
                <!--end::Label-->
                <select class="form-select form-select-solid drdn" id="valuation_type" name="valuation_type" data-control="select2" data-hide-search="true" data-placeholder="Valuation Type">
            ...
            <div class="row g-9 mb-8">
                ...
                <!--end::Label-->
                <select class="form-select form-select-solid drdn" id="battery_faulty" name="battery_faulty" data-control="select2" data-hide-search="true" data-placeholder="Battery Faulty">
                    <option value="">Pilih Battery Faulty</option>
            ...
            <div class="row">
                <!--begin::Label-->
                <img class="col-md-3" src="" alt="" id="gambar">
                <div class="col-md-3" id="gambar1"></div>
                <div class="col-md-3" id="gambar2"></div>
                <div class="col-md-3" id="gambar3"></div>
            </div>
            <!-- checbox -->
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    $('body').on('click', '.btn-detail', function() {
        ...
        // console.log(`{{url('detailitem/${dataid}')}}`)
      ...
      $('#gambar').attr("src","");
      $.ajax({
            url: `{{url('detailitem/${dataid}')}}`,
            type: "GET", 
            success: function(response) {
                    console.log("data detail item",response)
                    jQuery.noConflict()
                    ...
                    {
                        // $('#gambar').hide()
                        $('#status').val("Menunggu keputusan")
                        var newGambar = 'ttd/'+response.data.signature
                        $('#gambar').attr("src",newGambar);
                    }
                    ...
                    else if(response.data.status =="C" && response.data.signature != null) 
                    {
    ...
    $('body').on('click', '#btn-reject', function() {
        ...
        Swal.fire({ 
            title: "Konfirmasi",
            text: "Apakah yakin menolak pengembalian item ini ?",
            icon: "warning",
            ...
            if (result.value) {
                $.ajax({
                    type: "POST", 
                    url: `{{ url('/actionitem/${id_laporan}/B') }}`,
                    data: {
                        "_token": "{{ csrf_token() }}",
                    },
                    beforeSend: function() {
    ...
    $('body').on('click', '#btn-approve', function() {
        ...
        Swal.fire({ 
            title: "Konfirmasi",
            text: "Apakah yakin menyetujui pengembalian item ini ?",
            icon: "warning",
            showCancelButton: true,
            ...
            if (result.value) {
                $.ajax({
                    type: "POST",
                    url: `{{ url('/actionitem/${id_laporan}/C') }}`,
                    data: formData,
                    contentType: false,
                    cache:false,
                    processData: false,
